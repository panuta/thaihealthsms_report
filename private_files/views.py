...
def _handle_basic(request, instance, field_name, filename):
...
def _handle_nginx(request, instance, field_name, filename):
    field_file  = getattr(instance, field_name)
    basename = os.path.basename(field_file.path)
    mimetype, encoding = mimetypes.guess_type(field_file.path)
    mimetype = mimetype or 'application/octet-stream'
...
def _handle_xsendfile(request, instance, field_name, filename):
    field_file  = getattr(instance, field_name)
    basename = os.path.basename(field_file.path)
    mimetype, encoding = mimetypes.guess_type(field_file.path)
    mimetype = mimetype or 'application/octet-stream'
...
def get_file(request, app_label, model_name, field_name, object_id, filename):
    ...
    if condition(request, instance):
        ...
        return METHOD(request, instance, field_name, filename)
    else:
